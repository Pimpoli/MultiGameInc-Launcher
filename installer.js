...
/**
 * downloadToBuffer(url, token, onProgress)
 * - Usa axios stream para reportar progreso por archivo cuando sea posible.
 * - onProgress({ type:'download', url, received, total }) será llamado repetidamente.
 */
async function downloadToBuffer(url, token, onProgress) {
  ...
  try {
    ...
    if (res.status !== 200) {
      // fallback a arraybuffer si status != 200 (o no stream)
      if (res.status === 404) throw new Error('404 Not Found');
      // try simple arraybuffer fallback
...
/** Ejecuta el jar instalador (java -jar ...) */
...
/** Intenta abrir el launcher de Minecraft (plataforma específica) */
function launchMinecraftLauncher() {
  return new Promise((resolve) => {
    try {
      ...
      console.warn('[installer] no se pudo abrir launcher automáticamente:', e.message || e);
      resolve(false);
    }
  });
}
...
/**
 * listGithubDirFiles: usa la API de GitHub para listar archivos en un directorio del repo.
 * Retorna array de { name, download_url } o [] en fallo.
 */
...
/**
 * getRepoPathPartsFromManifestUrl:
 */
function getRepoPathPartsFromManifestUrl(manifestUrl) {
  try {
    const u = new URL(manifestUrl);
    const parts = u.pathname.split('/').filter(Boolean); // [owner, repo, branch, ...path, filename]
    if (parts.length < 4) return null;
    const owner = parts[0], repo = parts[1];
    const repoPathParts = parts.slice(3, parts.length - 1); // drop branch and filename
...
/**
 * listInstallersFromRepo(manifestUrl, token?)
 */
...
/**
 * applyDownloads: mueve/extrae los archivos desde tmpDir -> .minecraft
 * ahora acepta progress(tmp) opcional para reportar etapa/copia por archivo
 */
...
/**
 * installVersionHandler(payload, progress?)
 * - payload: { manifestUrl, versionId, token, selectedInstallerName, installPath, ... }
 * - progress: optional function(progressObj) where progressObj has keys like:
 *    { stage, currentFile, fileIndex, fileCount, totalBytes, downloadedBytes, speedBytesPerSec, items }
 */
async function installVersionHandler(payload, progress = null) {
  ...
  // helper to report overall progress simple approximation
  let bytesTotalApprox = 0;
    ...
    for (let idx = 0; idx < filesDeclared.length; idx++) {
      ...
      try {
        ...
        const buf = await downloadToBuffer(fileUrl, token, (evt) => {
          // evt: { type:'download', url, received, total }
  ...
  // si no hay faltantes críticos -> aplicar inmediatamente (report progress)
...
/** Limpia tmpDir (por si el usuario cancela) */
